 ...
 *  `\r`, none of writeTermInput's "compose a new line" behavior. This is the
 *  interactive-terminal input path (#47): every keystroke, paste, and control
 *  sequence the browser's xterm produces — including Escape to interrupt — is
 *  forwarded here verbatim, exactly as a real keyboard would. Serialized
 *  through the session's write lock so it can never interleave with a
 *  concurrent writeTermInput (the server-side kickoff typing). */
